...
1. 一次多选或拖入多个 CSV，或选“追加词表”后继续导入。选“替换词库”会替换整批；首次导入自动移除示例。每份文件可在“设置这份 CSV 的列”中单独映射，所有文件的词一起背、一起抽取。
2. 展开左侧“云端词表”，创建同步码，并立即复制到安全的地方。
3. 点击“保存当前词表”。可以保存多份；从云端打开后再保存会更新该份，重新导入 CSV 后保存会新建一份。
4. 另一台设备打开同一网站，粘贴同步码并连接，选择已保存词表后点击“打开词表”。

云端保留每个 CSV 的文件名、原始表头、所有原始列和值、独立字段映射、星标。造句草稿不上传，打开另一份词表会提示并清空本页草稿。不是自动同步：修改星标后需再次保存。

追加保留原来的星标及当前造句草稿；点击“重新抽取”才会重新生成包含新词的练习。相同单词和相同文件名不会自动去重，以免丢掉不同例句或释义。任一文件出错时整批不导入，不会出现只导入前半批的情况。

单 CSV 的星标仍可一键下载。多 CSV 时，点击“导出星标”后选择各个文件分别下载，保留原始列、未映射列及原始行顺序，不强行拼接不同结构的 CSV。同名文件的导出名会加序号以便区分。

每个同步码最多保存 20 份词库、合计 40 MiB；每份词库最多包含 20 个 CSV，合计最多 20,000 个原始数据行，每个 CSV 最多 100 列、5 MiB。本地合并导入会检查 10 MiB 的总文件/结构化数据上限，云端快照（含名称及星标）也限制 10 MiB。JSON 表示可能比 CSV 更大，云端保存超限不会清空本地练习。

单 CSV 继续使用 `schemaVersion: 1`；多 CSV 使用 `schemaVersion: 2` 的 `sources` 数组。新版本可读取旧快照，不修改旧记录，不需要数据库迁移。星标使用累加原始行数的全局索引，空词行不会让其他 CSV 的星标错位。追加后再保存会新建合并词库，不覆盖原先的单份云端副本。
...
2026-09-07 初次云端发布验证：65 项测试通过，完整 lint、TypeScript 检查及构建通过。已部署服务通过 20,000 词、4,009,089 字节快照的真实往返、另一设备读取、不同同步码隔离、版本冲突及删除检查；测试词表已清理。

2026-09-08 多 CSV 更新验证：81 项测试通过，完整 lint、TypeScript 检查及构建通过。已部署 API 同时通过上述大词库往返及多 CSV 独立映射、原始空词行、重复单词和星标的跨客户端恢复验证；只使用合成数据，测试副本已清理。
